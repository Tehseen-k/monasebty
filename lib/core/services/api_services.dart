...
class ApiServices {
  Future<Dio> launchDio({bool isRegister=false}) async {
    Dio dio = Dio();
  ...
  Future<RequestResponse> post(
      {required String url, dynamic data, Map<String, dynamic>? params,bool isRegister=false}) async {
    try {
      print("here 1 $url");
      Dio dio = await launchDio(isRegister: isRegister);
      print("hee2 $data");
  ...
  RequestResponse _handleError(Response? response) {
    ...
    switch (response.statusCode) {
      ...
      default:
        return RequestResponse(false,
            error: response.data != null && response.data.isNotEmpty
                ? response.data['error']
                : 'Network Error',
            data: null);
    }
